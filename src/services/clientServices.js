...
export const cargarUsuarios = async () => {
  try {
    const response = await axios.get(
      "http://localhost:3900/api/user/mostrarUsuarios"
    );
    console.log(response.data);
...
export const registrarActivo = async (
  name,
  nameActive,
  subCategoria,
  description
  ...
  try {
    const response = await axios.post(
      "http://localhost:3900/api/activos/registrar",
      {
        name,
        nameActive,
        subCategoria,
    ...
    console.log("Error registrando usuario " + error + error.response.data);  
  }
};
